...
export class TournamentStore {
  ...
  generateKnockoutBracket() {
      ...
      // Always create Final + Semis
      const finalMatch = createMatch('F');
      const sf1 = createMatch('SF', finalMatch.id, 'top');
      const sf2 = createMatch('SF', finalMatch.id, 'bottom');
      newMatches.push(finalMatch, sf1, sf2);

      if (numPlayers === 4) {
          // Seed directly into SFs
          sf1.p1 = n(qualifiedPlayers[0]); sf1.p2 = n(qualifiedPlayers[1]);
          sf2.p1 = n(qualifiedPlayers[2]); sf2.p2 = n(qualifiedPlayers[3]);

      } else if (numPlayers === 8) {
          // Create QFs and seed directly
          ...
          [qf1, qf2, qf3, qf4].forEach((qf, i) => {
              qf.p1 = n(qualifiedPlayers[i * 2]);
              qf.p2 = n(qualifiedPlayers[i * 2 + 1]);
          });
          newMatches.push(qf1, qf2, qf3, qf4);

      } else {
          // Round of 16: create QFs first, then R16 matches feeding into them
          const qf1 = createMatch('QF', sf1.id, 'top');
          const qf2 = createMatch('QF', sf1.id, 'bottom');
          const qf3 = createMatch('QF', sf2.id, 'top');
          const qf4 = createMatch('QF', sf2.id, 'bottom');
          ...
          if (settings.mode === 'Round-Robin + Knockout' && groups.length === 4) {
              // Cross-group seeding: groups[0] vs groups[1], groups[2] vs groups[3]
              // 1st plays 4th of the other group, 2nd plays 3rd
              const gA = this.getGroupStandings(groups[0].id);
              const gB = this.getGroupStandings(groups[1].id);
              const gC = this.getGroupStandings(groups[2].id);
              const gD = this.getGroupStandings(groups[3].id);

              newMatches.push(
                  // QF1 feeders: A1 vs B4, B1 vs A4
                  seedMatch('Ro16', n(gA[0]), n(gB[3]), qf1.id, 'top'),
                  seedMatch('Ro16', n(gB[0]), n(gA[3]), qf1.id, 'bottom'),
                  // QF2 feeders: A2 vs B3, B2 vs A3
                  seedMatch('Ro16', n(gA[1]), n(gB[2]), qf2.id, 'top'),
                  seedMatch('Ro16', n(gB[1]), n(gA[2]), qf2.id, 'bottom'),
                  // QF3 feeders: C1 vs D4, D1 vs C4
                  seedMatch('Ro16', n(gC[0]), n(gD[3]), qf3.id, 'top'),
                  seedMatch('Ro16', n(gD[0]), n(gC[3]), qf3.id, 'bottom'),
                  // QF4 feeders: C2 vs D3, D2 vs C3
                  seedMatch('Ro16', n(gC[1]), n(gD[2]), qf4.id, 'top'),
                  seedMatch('Ro16', n(gD[1]), n(gC[2]), qf4.id, 'bottom'),
              );
          } else {
              // Generic R16: sequential seeding into QFs
              const qfs = [qf1, qf2, qf3, qf4];
              for (let i = 0; i < 4; i++) {
                  newMatches.push(
                      seedMatch('Ro16', n(qualifiedPlayers[i*4]), n(qualifiedPlayers[i*4+3]), qfs[i].id, 'top'),
                      seedMatch('Ro16', n(qualifiedPlayers[i*4+1]), n(qualifiedPlayers[i*4+2]), qfs[i].id, 'bottom')
                  );
              }
          }
      }
