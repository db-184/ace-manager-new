...
export class TournamentStore {
  ...
  generateKnockoutBracket() {
      ...
      if (settings.mode === 'Knockout Only') {
          // Random draw directly into the bracket
          ...
          // Pair groups first-vs-last: groups[0] vs groups[n-1], groups[1] vs groups[n-2], etc.
          // Use current standings (works even if not all matches are complete).
          if (groups.length === 0) return;

          const perGroup = Math.ceil(numPlayers / groups.length);
          const numPairs = Math.floor(groups.length / 2);

          // Build group pairs: (first, last), (second, second-to-last), ...
          ...
          if (numPlayers === 4) {
              // 2 groups, seed directly into SFs
              ...
              // Each group pair generates 2 QFs
              // Pair 0 → QF1 (→ SF1 top) & QF2 (→ SF1 bottom)
              // Pair 1 → QF3 (→ SF2 top) & QF4 (→ SF2 bottom)
              ...
              groupPairs.forEach(([groupA, groupB], pairIdx) => {
                  ...
                  // Top QF: 1st of first group vs 2nd of last group (positions 0 & 1)
                  // Bottom QF: 1st of last group vs 2nd of first group
              ...
              // Round of 16:
              // Each group pair generates 2 QFs and 4 R16 matches
              // Pattern per pair: gFirst[0] vs gLast[3], gLast[0] vs gFirst[3] (→ QF_a)
              //                   gFirst[1] vs gLast[2], gLast[1] vs gFirst[2] (→ QF_b)
              const sfSlots = [sf1, sf2];
              groupPairs.forEach(([groupA, groupB], pairIdx) => {
                  const gFirst = this.getGroupStandings(groupA.id);
                  const gLast  = this.getGroupStandings(groupB.id);
                  ...
                  // R16 matches: 1st vs 4th (cross), 2nd vs 3rd (cross)
                  newMatches.push(
                      seedMatch('Ro16', n(gFirst[0]), n(gLast[perGroup - 1]),  qfA.id, 'top'),
                      seedMatch('Ro16', n(gLast[0]),  n(gFirst[perGroup - 1]), qfA.id, 'bottom'),
                      seedMatch('Ro16', n(gFirst[1]), n(gLast[perGroup - 2]),  qfB.id, 'top'),
                      seedMatch('Ro16', n(gLast[1]),  n(gFirst[perGroup - 2]), qfB.id, 'bottom'),
                  );
              });
          }
      }
