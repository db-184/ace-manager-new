  ...
  generateKnockoutBracket() {
      ...
      if (settings.mode === 'Knockout Only') {
          ...
          if (numPlayers === 4) {
              ...
              // Each group pair → 2 QFs
              // QF_a (→ sf top): 1st of first group vs 2nd of last group
              // QF_b (→ sf bottom): 1st of last group vs 2nd of first group
              ...
              // Round of 16
              // Each group pair → 2 QFs + 4 R16 matches
              // R16 pattern: 1st vs last, last-1st vs first (cross-seeded)
              groupPairs.forEach(([groupA, groupB], pairIdx) => {
                  const gFirst = this.getGroupStandings(groupA.id);
                  const gLast  = this.getGroupStandings(groupB.id);
                  const sf = sfSlots[pairIdx];
