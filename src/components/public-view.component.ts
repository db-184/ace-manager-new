  ...
  template: `
    <div class="min-h-screen bg-slate-950 flex flex-col text-slate-200">
      ...
      <main class="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
                        ...
                        <table class="min-w-full divide-y divide-slate-800">
                            <thead class="bg-slate-950">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-center text-xs font-medium text-slate-500 uppercase tracking-wider" title="Matches: Played-Won-Lost">P-W-L</th>
                                    <th class="px-6 py-3 text-center text-xs font-medium text-slate-500 uppercase tracking-wider" title="Games Won-Lost">W-L (G)</th>
                                </tr>
                            </thead>
                            <tbody class="bg-slate-900 divide-y divide-slate-800">
                                <tr *ngFor="let stat of getStandings(group.id); let i = index" 
                                    [ngClass]="searchQuery() && stat.name.toLowerCase().includes(searchQuery().toLowerCase()) ? 'bg-[#ccff00]/10' : ''" 
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-white flex items-center">
                                        {{ stat.name }}
                                        <span *ngIf="i < advancingPerGroup()" class="ml-2 w-1.5 h-1.5 rounded-full bg-[#ccff00]"></span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-center font-bold text-[#ccff00]">{{ stat.points }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-slate-400 font-mono">{{ stat.matchesPlayed }}-{{ stat.matchesWon }}-{{ stat.matchesLost }}</td>
          ...
          <div *ngIf="activeTab() === 'matches'" class="animate-fade-in">
                  <!-- Group filter pills -->
                  <div class="flex flex-wrap gap-2 mb-6">
                      <button
                          (click)="selectedGroupFilter.set(null)"
                          [class]="selectedGroupFilter() === null ? 'bg-[#ccff00] text-black' : 'bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700'"
                          class="px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wide transition-all"
                      >All Groups</button>
                      <button
                          *ngFor="let group of store.groups()"
                          (click)="selectedGroupFilter.set(group.id)"
                          [class]="selectedGroupFilter() === group.id ? 'bg-[#ccff00] text-black' : 'bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700'"
                          class="px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wide transition-all"
                      >{{ group.name }}</button>
                  </div>
                  ...
                  <div *ngIf="filteredMatches().length === 0" class="col-span-full text-center py-20 bg-slate-900/50 rounded-xl border border-dashed border-slate-800">
                      <p class="text-slate-500 text-lg">No matches found.</p>
                  </div>
                  </div>
          </div>
                                  ...
                                  <div class="bg-slate-900 border rounded-lg shadow-lg overflow-hidden transition-all" 
                                      ...
                                      <div class="p-3 space-y-2">
                                          <div class="flex justify-between items-center text-sm p-1 rounded" 
                                               [ngClass]="{
                                                 'text-white': match.winner === match.p1,
                                                 'text-slate-400': match.winner !== match.p1,
                                                 'bg-[#ccff00]/10': isMatchHighlighted(match) && match.p1.toLowerCase().includes(searchQuery().toLowerCase())
                                               }">
                                          ...
                                          <div class="flex justify-between items-center text-sm p-1 rounded" 
                                               [ngClass]="{
                                                 'text-white': match.winner === match.p2,
                                                 'text-slate-400': match.winner !== match.p2,
                                                 'bg-[#ccff00]/10': isMatchHighlighted(match) && match.p2.toLowerCase().includes(searchQuery().toLowerCase())
                                               }">
                                   ...
                                   <div *ngIf="round !== 'F' && round !== '3rd'" class="absolute top-1/2 -right-12 w-12 h-px bg-slate-700 hidden md:block"></div>
                              </div>
                          </div>
                  </div>
          </div>
...
export class PublicViewComponent {
  ...
  advancingPerGroup = computed(() => {
    const settings = this.store.settings();
    const groups = this.store.groups();
    if (settings.mode !== 'Round-Robin + Knockout' || groups.length === 0) return 0;
    const numPlayers = settings.knockoutStart === 'Round of 16' ? 16
                     : settings.knockoutStart === 'Round of 8' ? 8 : 4;
    return Math.ceil(numPlayers / groups.length);
  });
  ...
  filteredMatches() {
      ...
      let rrMatches = this.store.matches();

      if (groupId) {
          rrMatches = rrMatches.filter(m => m.groupId === groupId);
      }

      const filtered = !q
          ? rrMatches
          : rrMatches.filter(m => m.p1.toLowerCase().includes(q) || m.p2.toLowerCase().includes(q));

      return filtered.slice().sort((a, b) => {
          // Finished matches first
          if (a.isFinished !== b.isFinished) return a.isFinished ? -1 : 1;
          // Among finished: most recently completed at the top
