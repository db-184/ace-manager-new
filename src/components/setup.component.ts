...
// Fixed: Corrected library name and class name
  ...
  template: `
    <div class="max-w-2xl mx-auto bg-slate-900 p-8 rounded-xl shadow-2xl border border-slate-800">
      ...
      <div class="space-y-8">
        ...
        <div *ngIf="settings.mode === 'Round-Robin + Knockout'" class="animate-fade-in bg-slate-800/50 p-4 rounded-lg border border-slate-700">
          ...
          <div class="grid grid-cols-3 gap-3">
            <button *ngFor="let stage of ['Round of 16', 'Round of 8', 'Semi-finals']"
               type="button"
               (click)="settings.knockoutStart = stage"
...
export class SetupComponent {
  ...
  async generateName() {
    // Note: Environment variables are accessed via process.env usually in Node.
    // In Browser, ensure you have a mechanism to provide this safely.
    ...
    try {
        ...
        const result = await model.generateContent(
          'Generate a creative, short, professional name for a tennis tournament. Just return the name string, nothing else.'
        );
        ...
        if (text) {
            this.settings.name = text.trim().replace(/^"|"$/g, '');
        }
    } catch (e) {
