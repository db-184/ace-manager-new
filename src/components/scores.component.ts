                        ...
                        <div class="p-6">
                            @if (editingMatchId() === match.id) {
                                ...
                                <div class="bg-slate-800/50 p-6 rounded-lg border border-slate-700 animate-fade-in">
                                    ...
                                    @if (tempWalkover) {
                                        <div class="mb-6">
                                            <label class="block text-sm font-medium text-slate-400 mb-2 uppercase tracking-wide">Select Winner (Walkover)</label>
                                ...
                                <div class="flex items-center justify-between">
                                    ...
                                    <div class="mx-8 flex flex-col items-center min-w-[120px]">
                                        @if(match.isFinished) {
                                            <span class="text-3xl font-black text-[#ccff00] tracking-widest font-mono drop-shadow-[0_0_8px_rgba(204,255,0,0.3)]">{{ match.score || '0-0' }}</span>
                            ...
                            <table class="min-w-full divide-y divide-slate-800">
                                <thead class="bg-slate-950">
                                    <tr>
                                        ...
                                        <th scope="col" class="px-6 py-3 text-center text-xs font-medium text-slate-500 uppercase tracking-wider" title="Matches Played">Played</th>
                                        <th scope="col" class="px-6 py-3 text-center text-xs font-medium text-slate-500 uppercase tracking-wider" title="Matches Won">Won</th>
                                        <th scope="col" class="px-6 py-3 text-center text-xs font-medium text-slate-500 uppercase tracking-wider" title="Matches Lost">Lost</th>
                                ...
                                <tbody class="bg-slate-900 divide-y divide-slate-800">
                                    @for (stat of getStandings(group.id); track stat.playerId; let i = $index) {
                                        <tr [ngClass]="i < advancingPerGroup() ? 'bg-[#ccff00]/5' : ''" class="hover:bg-slate-800/50 transition-colors">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-white flex items-center">
                                                {{ stat.name }}
                                                @if(i < advancingPerGroup()) { <span class="ml-2 w-2 h-2 rounded-full bg-[#ccff00]"></span> }
                                            </td>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-green-400">{{ stat.matchesWon }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-red-400">{{ stat.matchesLost }}</td>
...
export class ScoresComponent {
  ...
  proceedToKnockout() {
      if (confirm('Generate the Knockout Bracket now? Players currently in qualifying positions will be seeded in.')) {
